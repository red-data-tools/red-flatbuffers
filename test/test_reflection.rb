...
class TestReflection < Test::Unit::TestCase
  def setup
    data = Helper::Path.reflection_bfbs.open("rb", &:read)
  ...
  class TestSchema < self
    ...
    def test_advanced_features
      assert_equal(Reflection::AdvancedFeatures.new(0),
  ...
  class TestEnum < self
    ...
    def test_documentation
      assert_equal([" New schema language features " +
                    "that are not supported by old code generators."],
                   @enum.documentation)
    end
